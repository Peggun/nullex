...
// This file: <src/arch/x86_64/bootinfo/memory_map.rs>
// Portions copied from upstream:
//   https://github.com/rust-osdev/bootloader (commit 0c97c72)
//   Upstream original file: <src/bootinfo/mod.rs>
// Copyright (c) 2018 Philipp Oppermann
// Modifications: Removed unused features, and removed `cfg` for
// map_physical_memory and embedded it natively. Made externs unsafe. `mod
// memory_map` to be public. See THIRD_PARTY_LICENSES.md for full license texts
// and upstream details.
...
pub struct BootInfo {
	...
	/// regions that the bootloader uses in the memory map before passing it to
	/// the kernel. Regions marked as usable can be freely used by the kernel.
	...
	/// addresses can easily break memory safety and cause undefined behavior.
	/// Only frames reported as `USABLE` by the memory map in the `BootInfo`
...
impl BootInfo {
	...
	/// Returns information about the thread local storage segment of the
	/// kernel.
	///
...
/// location. The additional `mem_size - file_size` bytes must be initialized
/// with zero.
