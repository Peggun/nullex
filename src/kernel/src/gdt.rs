...
pub fn init() {
	...
	// Initialize stacks
	let kernel_stack_start = VirtAddr::from_ptr(&raw const KERNEL_STACK as *const _);
	...
	let double_fault_stack_start =
		VirtAddr::from_ptr(&raw const DOUBLE_FAULT_STACK as *const _);
	unsafe {
		TSS.interrupt_stack_table[DOUBLE_FAULT_IST_INDEX as usize] =
			double_fault_stack_start + STACK_SIZE;
	}

	let page_fault_stack_start =
		VirtAddr::from_ptr(&raw const PAGE_FAULT_STACK as *const _);
	unsafe {
		TSS.interrupt_stack_table[PAGE_FAULT_IST_INDEX as usize] =
			page_fault_stack_start + STACK_SIZE;
	}

	let timer_ist_stack_start =
		VirtAddr::from_ptr(&raw const TIMER_IST_STACK as *const _);
	unsafe {
		TSS.interrupt_stack_table[TIMER_IST_INDEX as usize] = timer_ist_stack_start + STACK_SIZE;
	}
