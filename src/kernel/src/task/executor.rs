...
use x86_64::{instructions::interrupts, structures::paging::Translate, PhysAddr, VirtAddr};
use core::{arch::asm, sync::atomic::{AtomicBool, Ordering}, task::{Poll, Waker}};
...
use crate::{gdt::TSS, println, serial_println};
...
pub fn allocate_kernel_stack() -> usize {
    ...
    let stack_bottom = unsafe { alloc::alloc::alloc_zeroed(alloc::alloc::Layout::from_size_align(STACK_SIZE, 16).unwrap()) } as usize;
...
pub fn deallocate_kernel_stack(stack_top: usize) {
    ...
    unsafe { alloc::alloc::dealloc(stack_bottom as *mut u8, alloc::alloc::Layout::from_size_align(STACK_SIZE, 16).unwrap()) };
}
...
pub unsafe fn switch_to_process(process: &UserProcess) {
    ...
    unsafe {
        ...
        serial_println!("[Debug] TSS.RSP0 set to {:#x}", crate::gdt::TSS.privilege_stack_table[0].as_u64());
    }
...
pub fn translate_user_virtual_address(addr: VirtAddr) -> Option<PhysAddr> {
    ...
    let page_table_ptr = x86_64::registers::control::Cr3::read().0.start_address().as_u64() as *mut _;
    let mut page_table = unsafe { &mut *page_table_ptr };
    let mut mapper = unsafe { x86_64::structures::paging::OffsetPageTable::new(page_table, VirtAddr::new(0)) };
...
pub fn run_combined_executor() -> ! {
    loop {
        if SCHEDULER_TICK_PENDING.swap(false, Ordering::Acquire) {
            let mut executor = EXECUTOR.lock();
            for pid in executor.processes.keys() {
                if let Some(process_arc) = executor.processes.get(pid) {
                    let process = process_arc.lock();
                    let waker = ProcessWaker::new(*pid, executor.process_queue.clone(), process.state.clone());
                    waker.wake_by_ref();
                }
            }
            drop(executor);
            ...
            if let Some(pid) = executor.process_queue.pop() {
                if let Some(process_arc) = executor.processes.get(&pid) {
                    ...
                    if let Poll::Ready(result) = process.poll(&mut context) {
                        serial_println!("[Info] Process {} completed with {:?}", pid.get(), result);
