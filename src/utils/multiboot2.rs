...
use crate::{PHYS_MEM_OFFSET, println, utils::elf::{Elf32Shdr, Elf64Shdr, SHF_ALLOC}};
        ...
        while (*tag).r#type != MULTIBOOT_TAG_TYPE_END {
            ...
            match (*tag).r#type {
                ...
                MULTIBOOT_TAG_TYPE_MODULE => {
                    ...
                    println!("Module at 0x{:X}-0x{:X}. Command line {:?}", (*module).mod_start, (*module).mod_end, (*module).cmdline);
                },
                MULTIBOOT_TAG_TYPE_BASIC_MEMINFO => {
                    let mem = tag as *const MultibootTagBasicMemInfo;
                    println!("mem_lower = {:?}, mem_upper = {:?}", (*mem).mem_lower, (*mem).mem_upper);
                },
                    ...
                    while (entry_ptr as *const u8) < end {
                        ...
                        let region_kind = match mmap_entry.r#type {
                            ...
                            MULTIBOOT_MEMORY_NVS => {
                                bootloader::bootinfo::MemoryRegionType::AcpiNvs
                            }
                        ...
                        bl_map.add_region(bootloader::bootinfo::MemoryRegion {
                            range: FrameRange::new(base as u64, len as u64),
                            region_type: region_kind
                        });

                        entry_ptr = ((entry_ptr as *const u8).wrapping_add(tag_mmap.entry_size as usize))
                ...
                MULTIBOOT_TAG_TYPE_FRAMEBUFFER => {
                    ...
                    match (*tagfb).common.framebuffer_type {
                        MULTIBOOT_FRAMEBUFFER_TYPE_INDEXED => {
                            ...
                            if !pal_ptr.is_null() && num > 0 {
                                ...
                                for idx in 0..num {
                                    ...
                                    let distance = b_inv.wrapping_mul(b_inv)
                        ...
                        MULTIBOOT_FRAMEBUFFER_TYPE_RGB => {
                            colour = ((1 << (*tagfb).details.rgb_fields.framebuffer_blue_mask_size) - 1) 
                    ...
                    // loops through width and height, could use two for loops, the kernel.c code says otherwise.
                    while i < (*tagfb).common.framebuffer_width && i < (*tagfb).common.framebuffer_height {
                        match (*tagfb).common.framebuffer_bpp {
                            8 => {
                                let pixel = fb.wrapping_add(((*tagfb).common.framebuffer_pitch * i + i).try_into().unwrap()) as *mut u8;
                                *pixel = colour as u8;
                            },
                            16 => {
                                let pixel = fb.wrapping_add(((*tagfb).common.framebuffer_pitch * i + 2 * i).try_into().unwrap()) as *mut u16;
                                *pixel = colour as u16;
                            },
                            24 => { 
                                let pixel = fb.wrapping_add(((*tagfb).common.framebuffer_pitch * i + 3 * i).try_into().unwrap()) as *mut u32;
                                *pixel = (colour & 0xffffff) | (*pixel & 0xff000000)
                            },
                            32 => {
                                let pixel = fb.wrapping_add(((*tagfb).common.framebuffer_pitch * i + 4 * i).try_into().unwrap()) as *mut u32;
                                *pixel = colour;
                            }
                            _ => {}
                        }
                ...
                MULTIBOOT_TAG_TYPE_LOAD_BASE_ADDR => {
                    let lb: *const MultibootTagLoadBaseAddr = tag as *const MultibootTagLoadBaseAddr;
            ...
            tag = (tag as *const u8).add((((*tag).size + 7) & !7).try_into().unwrap()) as *const MultibootTag;
