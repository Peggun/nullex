...
impl<'a, B: Unblock> PanicGuard<'a, B> {
	...
	fn try_block(state: &'a AtomicOnceState) -> Result<Self, TryBlockError> {
		// (guard:1) this acquire CAS syncs-with the acq-rel swap (guard:2) and the
		// acq-rel CAS (wait:2)
