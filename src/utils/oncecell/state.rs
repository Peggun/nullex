...
use crate::utils::oncecell::POISON_PANIC_MSG;
...
impl AtomicOnceState {
	...
	pub(crate) fn try_block(&self, order: Ordering) -> Result<(), TryBlockError> {
		let prev = match self
			.0
			.compare_exchange(UNINIT, WOULD_BLOCK, order, Ordering::Relaxed)
		{
			Ok(prev) => prev,
			Err(prev) => prev
		};
