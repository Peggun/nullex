...
use crate::utils::bitflags::traits::{Flag, Flags};
...
impl<B: Flags> Iterator for IterNames<B> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		while let Some(flag) = self.flags.get(self.idx) {
			...
			// yield both flags.
			// 2. When flags fully overlap, such as in convenience flags that are a
