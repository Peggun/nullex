...
pub trait Flags: Sized + 'static {
	...
	///
	/// Each yielded flags value will correspond to a defined named flag. Any
	/// unknown bits will be yielded together as a final flags value.
	...
	///
	/// This method is like [`Flags::iter`], except only yields bits in
	/// contained named flags. Any unknown bits, or bits not corresponding to a
	/// contained flag will not be yielded.
	...
	/// unknown bits set. `remove` won't truncate `other`, but the `!` operator
	/// will.
	...
	///
	/// This method is not equivalent to `self & !other` when `other` has
	/// unknown bits set. `difference` won't truncate `other`, but the `!`
	/// operator will.
	#[must_use]
...
// Not re-exported: prevent custom `Bits` impls being used in the `bitflags!`
// macro, or they may fail to compile based on crate features
pub trait Primitive {}
...
pub trait BitFlags: ImplementedByBitFlagsMacro + Flags {
	...
	/// An iterator over the raw names and bits for enabled flags in an instance
	/// of the type.
...
/// A marker trait that signals that an implementation of `BitFlags` came from
/// the `bitflags!` macro.
///
/// There's nothing stopping an end-user from implementing this trait, but we
/// don't guarantee their manual implementations won't break between
