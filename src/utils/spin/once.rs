...
use core::{cell::UnsafeCell, fmt, marker::PhantomData, mem::MaybeUninit, sync::atomic::Ordering};

/// A primitive that provides lazy one-time initialization.
///
/// Unlike its `std::sync` equivalent, this is generalized such that the closure
/// returns a value to be stored by the [`Once`] (`std::sync::Once` can be
/// trivially emulated with `Once`).
///
/// Because [`Once::new`] is `const`, this primitive may be used to safely
/// initialize statics.
///
/// # Examples
///
...
mod status {
	...
	impl AtomicStatus {
		...
		pub fn store(&self, status: Status, ordering: Ordering) {
			// SAFETY: While not directly unsafe, this is safe because the value was
			// retrieved from a status, thus making transmutation safe.
		...
		pub fn get_mut(&mut self) -> &mut Status {
			// SAFETY: Since we know that the u8 inside must be a valid Status, we can
			// safely cast it to a &mut Status.
			unsafe { &mut *((self.0.get_mut() as *mut u8).cast::<Status>()) }
		}
	}
}
use self::status::{AtomicStatus, Status};
use crate::utils::spin::relax::{RelaxStrategy, Spin};
...
impl<T, R: RelaxStrategy> Once<T, R> {
	...
	///
	/// Note that in releases prior to `0.7`, this function had the behaviour of
	/// [`Once::poll`].
	///
	/// # Panics
	///
	...
	/// Like [`Once::get`], but will spin if the [`Once`] is in the process of
	/// being initialized. If initialization has not even begun, `None` will be
	/// returned.
	///
	...
	pub fn poll(&self) -> Option<&T> {
		loop {
			// SAFETY: Acquire is safe here, because if the status is COMPLETE, then we want
			// to make sure that all memory accessed done while initializing that value,
			// are visible when we return a reference to the inner data after this load.
...
impl<T, R> Once<T, R> {
	...
	///
	/// While this method itself is safe, accessing the pointer before the
	/// [`Once`] has been initialized is UB, unless this method has already
	/// been written to from a pointer coming from this method.
	...
	pub fn get(&self) -> Option<&T> {
		// SAFETY: Just as with `poll`, Acquire is safe here because we want to be able
		// to see the nonatomic stores done when initializing, once we have loaded and
		// checked the status.
		match self.status.load(Ordering::Acquire) {
	...
	/// Returns a reference to the inner value on the unchecked assumption that
	/// the  [`Once`] has been initialized.
	///
	/// # Safety
	///
	/// This is *extremely* unsafe if the `Once` has not already been
	/// initialized because a reference to uninitialized memory will be
	/// returned, immediately triggering undefined behaviour (even if the
	/// reference goes unused). However, this can be useful in some instances
	...
	/// initialized.
	///
	/// Because this method requires a mutable reference to the [`Once`], no
	/// synchronization overhead is required to access the inner value. In
	/// effect, it is zero-cost.
	...
	///
	/// Because this method requires ownership of the [`Once`], no
	/// synchronization overhead is required to access the inner value. In
	/// effect, it is zero-cost.
	...
	///
	/// This is done using [`Acquire`](core::sync::atomic::Ordering::Acquire)
	/// ordering, and therefore it is safe to access the value directly via
...
impl<'a> Drop for Finish<'a> {
	fn drop(&mut self) {
		// While using Relaxed here would most likely not be an issue, we use SeqCst
		// anyway. This is mainly because panics are not meant to be fast at all, but
		// also because if there were to be a compiler bug which reorders accesses
		// within the same thread, where it should not, we want to be sure that the
		// panic really is handled, and does not cause additional problems. SeqCst
		// will therefore help guarding against such bugs.
