...
// This file: <src/utils/spin/rwlock.rs>
// Portions copied from upstream:
//   https://github.com/zesterer/spin-rs (commit 71a1d8e)
//   Upstream original file: <src/rwlock.rs>
...
/// the [`RwLockUpgradableGuard::upgrade`] and
/// [`RwLockUpgradableGuard::try_upgrade`] functions. Writable or upgradeable
/// guards can be downgraded through their respective `downgrade` functions.
///
/// Based on Facebook's
...
/// then no writers will ever get a chance. Using an upgradeable lock guard can
/// *somewhat* alleviate this issue as no new readers are allowed when an
/// upgradeable guard is held, but upgradeable guards can be taken when there
...
/// [`RwLockWriteGuard`].
///
/// No writers or other upgradeable guards can exist while this is in scope. New
/// reader creation is prevented (to alleviate writer starvation) but there may
/// be existing readers when the lock is acquired.
///
/// When the guard falls out of scope it will release the lock.
...
impl<T, R> RwLock<T, R> {
	...
	/// current thread has acquired a write lock, and reading requires either a
	/// read or write lock.
	///
	/// # Example
	/// ```
...
impl<T: ?Sized, R: RelaxStrategy> RwLock<T, R> {
	...
	/// Obtain a readable lock guard that can later be upgraded to a writable
	/// lock guard. Upgrades can be done through the
	/// [`RwLockUpgradableGuard::upgrade`](RwLockUpgradableGuard::upgrade)
	/// method.
	#[inline]
...
impl<T: ?Sized, R> RwLock<T, R> {
	...
	/// Attempt to lock this rwlock with exclusive write access.
	///
	/// Unlike [`RwLock::try_write`], this function is allowed to spuriously
	/// fail even when acquiring exclusive write access would otherwise
	/// succeed, which can result in more efficient code on some platforms.
	#[inline]
...
impl<'rwlock, T: ?Sized> RwLockReadGuard<'rwlock, T> {
	...
	///
	/// Note that this function will permanently lock the original lock for all
	/// but reading locks.
	///
	/// ```
	/// let mylock = spin::RwLock::new(0);
...
impl<'rwlock, T: ?Sized, R> RwLockUpgradableGuard<'rwlock, T, R> {
	...
	fn try_upgrade_internal(self, strong: bool) -> Result<RwLockWriteGuard<'rwlock, T, R>, Self> {
		...
		.is_ok()
		{
			...
			// Forget the old guard so its destructor doesn't run (before mutably aliasing
			// data below)
			mem::forget(self);
	...
	/// Tries to upgrade an upgradeable lock guard to a writable lock guard.
	///
	/// Unlike [`RwLockUpgradableGuard::try_upgrade`], this function is allowed
	/// to spuriously fail even when upgrading would otherwise succeed, which
	/// can result in more efficient code on some platforms.
	#[inline]
	...
	/// Cannot fail and is guaranteed not to spin.
	///
	/// ```
...
impl<'rwlock, T: ?Sized, R> RwLockWriteGuard<'rwlock, T, R> {
	...
	/// Leak the lock guard, yielding a mutable reference to the underlying
	/// data.
	///
	/// Note that this function will permanently lock the original lock.
	///
