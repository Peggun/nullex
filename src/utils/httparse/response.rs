...
use crate::{serial_println, utils::httparse::{headers::ResponseHeaders, url::ParsedUrl}};
...
pub fn url_looks_like_download(path: &str) -> bool {
    ...
    matches!(
        ext.as_str(),
        "bin" | "exe" | "zip" | "tar" | "gz" | "bz2" | "xz" | "7z"
        | "iso" | "img" | "dmg"
        | "pdf" | "doc" | "docx" | "xls" | "xlsx" | "ppt" | "pptx"
        | "mp3" | "mp4" | "mkv" | "avi" | "mov" | "flac" | "wav"
        | "jpg" | "jpeg" | "png" | "gif" | "webp" | "elf"
    )
}
...
mod tests {
    use super::*;
    use crate::utils::httparse::{headers::ResponseHeaders, url::ParsedUrl};
    use crate::utils::ktest::TestError;
 
    fn make_headers(content_type: Option<&str>, is_attachment: bool, filename: Option<&str>) -> ResponseHeaders {
        ResponseHeaders {
            status_code: 200,
            content_length: None,
    ...
    fn filename_from_url_with_query() -> Result<(), TestError> {
        let h = make_headers(None, false, None);
        assert_eq!(resolve_filename(&h, &url("/files/setup.exe?v=2")), "setup.exe");
