...
use crate::{fs, println, serial, serial_println, task::{executor::{self, CURRENT_PROCESS, EXECUTOR}, OpenFile, Process, ProcessState}};
        ...
        if let Some(open_file) = process.open_files.get_mut(&fd) {
            ...
            fs::with_fs(|fs| {
                if let Ok(file) = fs.get_file(path) {
                    let bytes_to_read = core::cmp::min(len, file.content.len().saturating_sub(offset));
                    if bytes_to_read > 0 {
