...
use crate::{fs::{self, resolve_path}, println, programs::nulx::{errors::NulxInputError, eval::eval_expr, lexer::lexer, parser::funcs_parser}};
...
pub fn run(args: &[&str]) {
    ...
    let src = fs::with_fs(|fs| -> Result<String, NulxInputError> { match fs.read_file(&path) {
        Ok(content) => {
            let s = String::from_utf8_lossy_owned(content.to_vec());
            Ok(s)
        },
        Err(_) => {
            Err(NulxInputError::NoSuchFile(path))
        },
    }
    }).unwrap();
