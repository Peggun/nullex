...
impl IoApic {
    ...
    pub unsafe fn init(&mut self, offset: u8, dest_apic: u8) {
        serial_println!("[IOAPIC] Starting init, offset={}, dest_apic={}", offset, dest_apic);

        let version_reg = self.regs.read(VERSION);
        let max_redir = ((version_reg >> 16) & 0xFF) as u8;
        serial_println!("[IOAPIC] Version {:#X}, max redir entries = {}", version_reg, max_redir);
        ...
        serial_println!("[IOAPIC] IRQ1 → vector {}, dest_apic {}", offset + 1, dest_apic);
    }
